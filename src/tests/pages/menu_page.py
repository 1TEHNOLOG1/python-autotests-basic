...
class MenuPage:
    def __init__(self, driver):
        ...
        self.slider_1 = (
            By.XPATH,
            "(//span[@class ='ui-slider-handle ui-state-default ui-corner-all'])[1]",
        )
    ...
    def filter_price(self):
        ...
        actions.click_and_hold(slider_1_element).move_by_offset(
            xoffset=50, yoffset=0
        ).perform()
        slider_2_element = self.driver.find_element(*self.slider_2)
        actions.click_and_hold(slider_2_element).move_by_offset(
            xoffset=-50, yoffset=0
        ).perform()
