...
def test_check_stars(page: Page):
    ...
    with allure.step('Проверка звезд в репозиториях '):
        with allure.step('Проверка звезд в репозиториях'):
            stars = [int(element.get_attribute("aria-label").split()[0]) > 20000 for element in
                     page.query_selector_all("//*[contains(@aria-label,'stars')]")]
            print(stars)
            assert all(stars)
...
def test_skillbox_profession(page: Page):
    ...
    page.wait_for_selector("//*[@class = 'courses-block courses-section__block']", timeout=3000)
    profession1 = page.text_content("(//*[@class = 'ui-product-card-main__title t t--2'])[1]")
    profession2 = page.text_content("(//*[@class = 'ui-product-card-main__title t t--2'])[2]")
