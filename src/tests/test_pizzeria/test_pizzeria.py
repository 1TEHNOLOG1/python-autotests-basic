...
class TestPizza:
    ...
    def test_use_promo(self, set_up_browser):
        ...
        with allure.step("Проверка применения купона "):
            assert (
                bucket_page.get_final_price() == bucket_page.get_original_price() * 0.90
            )
    ...
    def test_use_invalid_promo(self, set_up_browser):
        ...
        with allure.step("Добавление товара в корзину"):
            ...
            WebDriverWait(driver, 3).until(
                EC.presence_of_element_located((By.XPATH, "//div[@class='inner']"))
            )
            BucketPage(driver)
            bucket_page = BucketPage(driver)
            bucket_page.go_to_bucket()
        ...
        with allure.step("Проверка применения купона "):
            original_price = driver.find_element(
                By.XPATH, "(//span[@class ='woocommerce-Price-amount amount'])[2]"
            ).text
            original_price = float(original_price.replace("₽", "").replace(",", "."))
            final_price = driver.find_element(
                By.XPATH, "(//span[@class ='woocommerce-Price-amount amount'])[3]"
            ).text
            ...
            alert = driver.find_element(
                By.XPATH, "//*[contains(@class,'woocommerce-error')]"
            ).text
    ...
    def test_double_use_promo(self, set_up_browser):
        ...
        with allure.step("Проверка применения промокода"):
            assert (
                "ЗАКАЗ ПОЛУЧЕН"
                in driver.find_element(By.XPATH, "//h2[@class='post-title']").text
            )
    ...
    def test_bonus_program(self, set_up_browser):
        ...
        with allure.step("Проверка"):
            assert (
                "Ваша карта оформлена!" in driver.find_element(By.ID, "bonus_main").text
            )
    ...
    def test_bonus_program_negativ(self, set_up_browser, phone):
        ...
        with allure.step("Проверка"):
            assert (
                ...
                in driver.find_element(By.ID, "bonus_content").text
            )
    ...
    def test_delite_items_in_backed(self, set_up_browser):
        ...
        with allure.step("Переход на страницу - корзина"):
            bucket_page = BucketPage(driver)
            bucket_page.go_to_bucket()
            WebDriverWait(set_up_browser, 2).until(
        ...
        with allure.step("Проверка удаления товара из корзины"):
            assert (
                driver.find_element(By.CSS_SELECTOR, "p.cart-empty").text
                == "Корзина пуста."
            )
    ...
    def test_filter_price(self, set_up_browser):
        ...
        with allure.step("Проверка фильтрации товаров по цене"):
            ...
            WebDriverWait(set_up_browser, 3).until(
